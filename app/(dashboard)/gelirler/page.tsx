...
async function addIncome(formData: FormData) {
  ...
  if (amount && categoryId && date) {
    await db.prepare(`INSERT INTO transactions (type, amount, categoryId, description, date, profileId) VALUES ('INCOME', ?, ?, ?, ?, ?)`).run(amount, categoryId, description, date, session.profileId);
...
async function updateIncome(formData: FormData) {
  ...
  if (id && amount && categoryId && date) {
    await db.prepare(`UPDATE transactions SET amount = ?, categoryId = ?, date = ?, description = ? WHERE id = ? AND profileId = ? AND type = 'INCOME'`)
...
async function deleteIncome(formData: FormData) {
  ...
  const id = formData.get('id');
  await db.prepare(`DELETE FROM transactions WHERE id = ? AND type = 'INCOME' AND profileId = ?`).run(id, session.profileId);
  revalidatePath('/gelirler');
  revalidatePath('/');
}
...
export default async function GelirlerPage({ searchParams }: { searchParams: any }) {
  ...
  const categories = await db.prepare(`SELECT * FROM categories WHERE type = 'INCOME' AND (profileId = ? OR profileId IS NULL) ORDER BY name ASC`).all(session.profileId) as any[];
  ...
  const incomes = await db.prepare(query).all(...queryParams) as any[];
      ...
      <div className="glass-card p-4">
        <form method="GET" className="flex flex-col sm:flex-row gap-4 items-end">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <a href="/gelirler?month=all&categoryId=all" className="px-6 py-2.5 rounded-xl font-bold text-[#8e95a5] hover:text-white border border-[#ffffff14] transition-colors text-center w-full sm:w-auto">Sıfırla</a>
          </div>
        </form>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody>
                {incomes.length === 0 ? (
                  ...
                  incomes.map((inc) => (
                    editId === inc.id.toString() ? (
                      <tr key={inc.id} className="border-b border-[#ffffff14] bg-[rgba(0,255,136,0.05)]">
                        <td colSpan={5} className="p-3">
                          <form action={updateIncome} className="flex flex-col xl:flex-row gap-3 items-center w-full">
                            <input type="hidden" name="id" value={inc.id} />
                            <input type="date" name="date" required defaultValue={inc.date.split('T')[0]} className="glass-input p-2 rounded-lg w-full xl:w-auto text-sm" />
                            <select name="categoryId" required defaultValue={inc.categoryId} className="glass-input p-2 rounded-lg w-full xl:w-48 text-sm">
                              {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                            </select>
                            <input type="text" name="description" defaultValue={inc.description || ''} className="glass-input p-2 rounded-lg w-full xl:flex-1 text-sm" placeholder="Kısa Açıklama" />
                            <input type="number" step="0.01" name="amount" required defaultValue={inc.amount} className="glass-input p-2 rounded-lg w-full xl:w-32 font-bold text-[var(--color-neon-green)]" />
                            <div className="flex gap-2 w-full xl:w-auto justify-end">
                              ...
                              <a href={`/gelirler?month=${monthFilter}`} className="p-2 bg-[#ffffff14] text-white rounded-lg hover:scale-105 transition-all"><X className="w-5 h-5"/></a>
                            </div>
                          </form>
                        </td>
                      </tr>
                      ...
                      <tr key={inc.id} className="border-b border-[#ffffff0a] hover:bg-[#ffffff05] transition-colors">
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <a href={`/gelirler?month=${monthFilter}&edit=${inc.id}`} className="p-2 text-[#8e95a5] hover:text-[var(--color-neon-blue)] hover:bg-[rgba(0,240,255,0.1)] rounded-lg transition-colors" title="Düzenle">
