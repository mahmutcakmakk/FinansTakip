...
async function getDashboardData(profileId: number) {
  ...
  const todayIncome = await db.prepare(`SELECT SUM(amount) as total FROM transactions WHERE type = 'INCOME' AND createdAt LIKE ? AND profileId = ?`).get(`${today}%`, profileId) as {total: number|null};
  const todayExpense = await db.prepare(`SELECT SUM(amount) as total FROM transactions WHERE type = 'EXPENSE' AND createdAt LIKE ? AND profileId = ?`).get(`${today}%`, profileId) as {total: number|null};
